...
export const ContractSigningStep: React.FC<ContractSigningStepProps> = ({
  ...
  const refreshContract = async (options: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      const found = await contractsApi.getByProjectPropose(proposal.id, 1);
          ...
          <div>
            ...
            {activeContract ? (
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                <div className="flex justify-between">
                  <Text strong>Dự án:</Text>
                  <Text>{proposal.project?.name || "N/A"}</Text>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <Text strong>Số tiền đầu tư:</Text>
                  <Text className="text-green-600 font-semibold">
                    {proposal.investment_amount?.toLocaleString("vi-VN")} VND
                  </Text>
                </div>
                <div className="flex justify-between">
                  <Text strong>Trạng thái:</Text>
