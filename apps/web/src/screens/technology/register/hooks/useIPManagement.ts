...
export const useIPManagement = ({ technologyId, onIPDetailsChange }: UseIPManagementProps = {}) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string>('');
  const [savedIPDetails, setSavedIPDetails] = useState<IntellectualProperty[]>([]);

  // Convert API IP to form IP detail
  const convertAPIToFormIP = useCallback((apiIP: IntellectualProperty): IPDetail => ({
    ipType: apiIP.type,
    ipNumber: apiIP.code,
    status: apiIP.status,
    territory: apiIP.technology, // Assuming territory maps to technology field
  }), []);

  // Convert form IP detail to API format
  const convertFormToAPIIP = useCallback((formIP: IPDetail, techId?: string): Partial<IntellectualProperty> => ({
    technology: formIP.territory || techId || '',
  ...
  const loadIPDetails = useCallback(async (techId: string) => {
    if (!techId) return;
  ...
  const saveIPDetail = useCallback(async (ipDetail: IPDetail, techId?: string): Promise<IntellectualProperty | null> => {
    setLoading(true);
    setError('');
  ...
  const saveAllIPDetails = useCallback(async (ipDetails: IPDetail[], techId?: string): Promise<IntellectualProperty[]> => {
    setLoading(true);
    setError('');
  ...
  const updateIPDetail = useCallback(async (ipId: string, ipDetail: IPDetail): Promise<IntellectualProperty | null> => {
    setLoading(true);
    setError('');
    ...
    try {
      ...
      setSavedIPDetails(prev => prev.map(ip => ip.id === ipId ? result : ip));
      return result;
    } catch (err: any) {
  ...
  const saveAsDraft = useCallback(async (ipDetails: IPDetail[], techId?: string): Promise<boolean> => {
  ...
  const loadFromDraft = useCallback((techId?: string): IPDetail[] => {
    try {
      const draftKey = `ip_draft_${techId || technologyId || 'new'}`;
  ...
  // Clear draft data
  const clearDraft = useCallback((techId?: string) => {
    try {
      const draftKey = `ip_draft_${techId || technologyId || 'new'}`;
