...
const mockTechnologies = [
  {
    ...
    public_summary:
      "Công nghệ AI nhận dạng hình ảnh tiên tiến sử dụng deep learning",
  ...
  },
  {
    ...
    pricing_type: "ASK",
    owners: [
      { owner_name: "Nguyễn Văn B", ownership_percentage: 60 },
      { owner_name: "Công ty XYZ", ownership_percentage: 40 },
    ],
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (category) {
      filteredTechnologies = filteredTechnologies.filter(
        (tech) => tech.category_name === category
      );
    }
    ...
    if (trl_level) {
      ...
      filteredTechnologies = filteredTechnologies.filter(
        (tech) => tech.trl_level >= min && tech.trl_level <= max
      );
    }
