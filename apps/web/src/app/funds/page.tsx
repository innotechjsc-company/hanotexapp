...
export default function FundsPage() {
  ...
  const [totalInvestmentGoal, setTotalInvestmentGoal] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (!isMounted) return;
        setActiveCount((activeRes.totalDocs as number) || 0);
        setPendingCount((pendingRes.totalDocs as number) || 0);
        setFundsCount((fundsRes.totalDocs as number) || 0);
        setActiveProjects(
          ((activeListRes.data as any) ||
            (activeListRes.docs as any) ||
            []) as Project[]
        );

        // Compute total goal money for projects that have an investment fund
        let page = 1;
        const limit = 50;
        let totalPages = 1;
        let sum = 0;
        do {
          ...
          const items: Project[] = (res.data as any) || (res.docs as any) || [];
          sum += items
            .filter((p: any) => !!p?.investment_fund)
            .reduce(
              (acc: number, p: any) => acc + (Number(p?.goal_money) || 0),
              0
            );
        ...
        if (isMounted) setTotalInvestmentGoal(sum);
      } catch (e: any) {
        if (isMounted) setError(e?.message || "Đã xảy ra lỗi khi tải dữ liệu");
      } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Kết nối các dự án công nghệ với nguồn vốn đầu tư, quỹ đầu tư và nhà
            đầu tư tiềm năng
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 text-center">
            <div className="text-3xl font-bold text-orange-600 mb-2">
              {isLoading ? (
                <span className="inline-block h-7 w-28 bg-gray-200 rounded animate-pulse" />
              ) : (
                new Intl.NumberFormat("vi-VN").format(totalInvestmentGoal)
              )}
            </div>
            <div className="text-gray-600">Tổng giá trị dự án có quỹ (VNĐ)</div>
          </div>
        </div>
